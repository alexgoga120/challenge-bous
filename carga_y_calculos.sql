...
WHERE client IS NOT NULL AND 
      contract_num IS NOT NULL AND 
      purchase_date IS NOT NULL AND 
      city IS NOT NULL AND 
      company IS NOT NULL AND 
      debit IS NOT NULL ORDER BY id;
...
SELECT pg_backend_pid(),
       COUNT(*) AS total_rows,
       SUM(challenge_bous.work_table.debit) AS total_debits
FROM challenge_bous.valid_debits 
INNER JOIN challenge_bous.work_table 
ON challenge_bous.valid_debits.fk_work_table_id = challenge_bous.work_table.id;
...
SELECT SUM(debit) AS debit,
       ROUND((SUM(debit) / (SELECT total_debits FROM challenge_bous.data_calculations WHERE run_id = pg_backend_pid() ) * 100), 2) AS percentage,
